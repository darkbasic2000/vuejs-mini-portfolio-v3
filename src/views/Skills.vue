<template>
  <v-sheet class="pa-5">
    <div class="text-h6 text-center">Adicionar Habilidade</div>
    <v-select name="level" label="Selecionar habilidades" @update:modelValue="addSkill" prepend-inner-icon="mdi-list-box"
      return-object :items="itens" item-title="title" item-value="icon">
...
const itens = ref([
  ...
  function addSkill(skill) {
    ...
    const added = store.addSkills(skill)    
    if(added) {
        itens.value = itens.value.filter(item => item.title !== skill.title)
    }   
